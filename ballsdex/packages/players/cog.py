...
from ballsdex.core.models import BallInstance, DonationPolicy, Player, Trade, TradeObject, balls, Ball
...
class Players(commands.GroupCog, group_name=settings.players_group_cog_name):
    ...
    async def rarity(self, interaction: discord.Interaction):
        ...
        if len(entries) == 0:
            await interaction.response.send_message("No balls found.", ephemeral=True)
            return

        source = FieldPageSource(entries, per_page=5, inline=False, clear_description=False)
        source.embed.description = "Brasil Rarity"
        source.embed.colour = discord.Colour.blue()
